...
def push_interactive_message(title: str, content: str, user_id: str) -> bool:
    """发送交互式消息（富文本）"""
    token = get_tenant_access_token()
    if not token:
        return False
...
def generate_summary(report_path: Path) -> str:
    ...
    for line in lines:
        ...
        if in_highlights and line.startswith("### "):
            highlight_count += 1
            if highlight_count <= 5:  # 最多 5 条
                summary_lines.append(line.strip())
            continue
        
        # 摘要内容
        if in_highlights and line.strip().startswith("- **摘要：**"):
        ...
        # 原文链接（已移除，节省字数）
        if in_highlights and line.strip().startswith("- **原文链接：**"):
            summary_lines.append("")
            continue
        
        # 遇到下一个大章节就停止
        if line.startswith("## ") and "今日重点动态" not in line:
            break
...
def push_full_report(report_path: Path, user_id: str) -> bool:
    """推送完整版报告（分段发送，纯文本适配微信）"""
    with open(report_path, 'r', encoding='utf-8') as f:
        report_content = f.read()
    
    # 清理 Markdown 格式，转为纯文本
    clean_content = clean_markdown(report_content)
...
def main():
    ...
    print(f"📤 推送到飞书用户：{FEISHU_USER_ID}")
